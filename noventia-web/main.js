(function () {
  ...
  /* ---------- Chat widget (talks to an n8n webhook) ---------- */
  function initChatWidget() {
    ...
    const form = $("[data-chat-form]", widget);
    const input = $("[data-chat-input]", widget);
    const webhookUrl = data.chatWebhookUrl;
    ...
    function getSessionId() {
      try {
        let id = localStorage.getItem("noventia_chat_session");
        if (!id) {
          id = (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()) + Math.random());
          localStorage.setItem("noventia_chat_session", id);
        }
        return id;
      } catch (e) {
    ...
    form.addEventListener("submit", e => {
      ...
      if (!webhookUrl || webhookUrl.indexOf("TU_WEBHOOK_URL") === 0) {
        addMessage("El chat todavía no está activado. Escríbenos directamente a " + (data.contactEmail || "andres@noventia.tech") + ".", "bot");
        return;
      }

      const typing = addMessage("Escribiendo…", "bot is-typing");
      ...
      fetch(webhookUrl, {
        ...
        .then(result => {
          typing.remove();
          addMessage(result.reply || "Perdona, no he podido procesar tu mensaje. Escríbenos a " + (data.contactEmail || "andres@noventia.tech") + ".", "bot");
        })
        .catch(() => {
          typing.remove();
          addMessage("Ha habido un problema de conexión. Escríbenos directamente a " + (data.contactEmail || "andres@noventia.tech") + ".", "bot");
        });
    });
  }
  ...
  function boot() {
    ...
    safe(initChatWidget, "initChatWidget");
